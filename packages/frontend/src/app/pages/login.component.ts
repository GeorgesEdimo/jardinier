...
@Component({
      ...
      <div class="auth-center">
        <div class="auth-card">
          ...
          <form (ngSubmit)="submit()" #f="ngForm" class="auth-form">
            ...
            <button type="submit" [disabled]="loading" class="btn-primary">Connexion</button>
  ...
  styles: [`
    ...
    .auth-center {
      position: relative; z-index: 1; display: grid; place-items: center; min-height: inherit; padding: 2rem 1rem;
    }
    .auth-card {
      width: 100%; max-width: 420px; padding: 2rem; border-radius: 16px;
      background: rgba(255,255,255,.08); border: 1px solid rgba(255,255,255,.25);
      backdrop-filter: blur(8px); color: #fff;
      box-shadow: 0 10px 30px rgba(0,0,0,.25);
    }
    .auth-card h2 { margin: 0 0 1rem; font-weight: 700; letter-spacing: .4px; }
    ...
    .auth-form input::placeholder { color: rgba(255,255,255,.8); }
...
export class LoginComponent {
  ...
  async submit() {
    ...
    try {
      ...
      this.error = e?.message || 'Identifiants invalides';
    } finally {
      this.loading = false;
    }
  }
